...
namespace TessBed
{
    public partial class MainForm : Form
    {
        ...
        private int VertexCombine(Vec3 position, int[] data, float[] weights)
        {
            var colors = new Color[] { Color.FromArgb(data[0]), Color.FromArgb(data[1]), Color.FromArgb(data[2]), Color.FromArgb(data[3]) };
            var rgba = new float[] {
            ...
            return Color.FromArgb((int)rgba[3], (int)rgba[0], (int)rgba[1], (int)rgba[2]).ToArgb();
        }
            ...
            for (int i = 0; i < _tess.ElementCount; i++)
            {
                ...
                for (int j = 0; j < _polySize; j++)
                {
                    ...
                    var v = new PolygonPoint {
                        ...
                        Color = Color.FromArgb(_tess.Vertices[index].Data)
                    };
                    poly.Add(v);
                }
                output.Add(poly);
